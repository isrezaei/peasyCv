...
import { deleteUploadedImage, uploadProfileImage } from "@/lib/api/uploads";
import { t } from "@/lib/i18n";
import { createId } from "@/lib/utils/id";
import { dataUrlToBlob } from "@/lib/utils/blob";
...
export function ProfileImageEditor({ size = "128px" }: ProfileImageEditorProps) {
  ...
  const handleCropConfirm = async (croppedDataUrl: string) => {
    const original = pendingImageSrc;
    if (!original) return;
    const previousAssetId = personalInfo.profileImage?.id;
    setPendingImageSrc(null);
    setBusy(true);
    try {
      // Upload to cloud storage so the resume stores a URL (not embedded base64),
      // replacing the prior asset if there was one.
      ...
      // Graceful fallback: keep the image inline so the feature still works
  ...
  const handleRemove = async () => {
    const assetId = personalInfo.profileImage?.id;
    removeProfileImage();
    if (assetId) {
      try {
        await deleteUploadedImage(assetId);
      } catch {
        /* best-effort cleanup; the store removal already happened */
      }
    }
  };
